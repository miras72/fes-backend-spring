...
public class JobServiceImpl implements JobService {
	...
	@Override
	public boolean scheduleOneTimeJob(String jobName, String subjectName, Long id,
		...
		String keySubjectNameValue = subjectName;
		Long keyIdValue = id;

		JobDetail jobDetail = JobUtil.createJob(jobClass, false, context, jobKey, groupKey, keySubjectNameValue,
				keyIdValue);
	 ...
	 */
	@Override
	public boolean scheduleCronJob(String jobName, String subjectName, Long id, Class<? extends QuartzJobBean> jobClass,
		...
		logger.info("Creating trigger for key: " + jobKey + "(" + subjectName + ")" + " at date :" + date);
		...
		try {
			Scheduler scheduler = schedulerFactoryBean.getScheduler();
			Date dt = scheduler.scheduleJob(jobDetail, cronTriggerBean);
			logger.info("Job with key jobKey: " + jobKey + "(" + subjectName + ")" + " and group: " + groupKey
					+ " scheduled successfully for date: " + dt);
			return true;
		} catch (SchedulerException e) {
			logger.error("SchedulerException while scheduling job with key: " + jobKey + "(" + subjectName + ")"
	 ...
	 */
	@Override
	public boolean updateCronJob(String jobName, String subjectName, Date date, String cronExpression) {
		logger.info("Request received for updating cron job.");

		String jobKey = jobName;

		logger.info("Parameters received for updating cron job with jobKey: " + jobKey + "(" + subjectName + ")"
				+ ", date: " + date);
		try {
			// Trigger newTrigger = JobUtil.createSingleTrigger(jobKey, date,
			// SimpleTrigger.MISFIRE_INSTRUCTION_RESCHEDULE_NEXT_WITH_REMAINING_COUNT);
			...
			Date dt = schedulerFactoryBean.getScheduler().rescheduleJob(TriggerKey.triggerKey(jobKey), newTrigger);
			logger.info("Trigger associated with jobKey: " + jobKey + "(" + subjectName + ")"
					+ " rescheduled successfully for date: " + dt);
			return true;
		} catch (Exception e) {
			logger.error("SchedulerException while updating cron job with jobKey: " + jobKey + " message: "
					+ e.getMessage());
			logger.error("StackTrace: ", e);
			return false;
		}
	...
	public Map<String, Object> getJob(String jobName) {
		...
		try {
			...
			map.put("jobName", jobName);
			map.put("groupName", groupKey);
			map.put("scheduleTime", scheduleTime);
			map.put("lastFiredTime", lastFiredTime);
			map.put("nextFireTime", nextFireTime);

			logger.info("Job details:");
			logger.info("Job Name: " + jobName + ", Group Name: " + groupKey + ", Next Fire Time: " + nextFireTime);
		} catch (SchedulerException e) {
